...
WRITE_CSTRING	PROC
				...
				; Your code goes here.
				PUSH	{lr}
				MOV		r10,#0				; r10 is the index of the string.
				CMP		r4,#0				; if the r4 is 0, it is top line		
				BEQ		topLine
				; then it is bottom line.
				MOV		r4, #0xC0		; 1100 0000 0x40 is the bottom left DDRAM address.
				BL		WRITE_COMMAND	; this will set DDRAM address.
...
topLine			; it is the top line.
				MOV		r4, #0x80		; 1000 0000 0x00 is the top left DDRAM address.
				BL		WRITE_COMMAND	; this will set DDRAM address.
				BL		DELAY_2_MS
...
loopString		LDRB	r4,[r5,r10]		; r4 is the value of Asci code
				CMP		r4,#0			; if r4=0, loop end
				BEQ		done
				BL		WRITE_DATA
				BL		DELAY_2_MS
				ADD		r10,r10,#1
...
WRITE_BYTE		PROC
				...
				LSR	r4,#4		; logical shift right so the MS nybble is saved.	
				BL	WRITE_LS_NYBBLE
